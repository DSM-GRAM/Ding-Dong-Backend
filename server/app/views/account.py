...
class SignUp(BaseResource):
    ...
    def post(self):
        ...
        if UserModel.query.filter_by(id=id).first() is not None:
            return Response('ID duplicated', 409)

        if (menstruation_period >= 26) and (menstruation_period <= 32):
            user = UserModel(id=id, password=hashed_pw, menstruation_period=menstruation_period,
                              is_rhythm_contraception=False, is_standard_contraception=True)
        else:
            user = UserModel(id=id, password=hashed_pw, menstruation_period=menstruation_period,
                              is_rhythm_contraception=True, is_standard_contraception=False)
